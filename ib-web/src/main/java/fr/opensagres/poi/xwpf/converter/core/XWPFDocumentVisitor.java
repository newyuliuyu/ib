...
/**
 * ClassName: XWPFDocumentVisitor <br/>
 * Function:  ADD FUNCTION. <br/>
 * Reason:  ADD REASON(可选). <br/>
 * date: 18-9-29 上午10:51 <br/>
 *
 * @author liuyu
 * @version v1.0
 * @since JDK 1.7+
...
public abstract class XWPFDocumentVisitor<T, O extends Options, E extends IXWPFMasterPage>
    ...
    protected void visitRunCTOMathImpl(XWPFParagraph paragraph, CTOMathImpl ctoMath, T paragraphContainer) throws Exception {
    ...
    protected boolean isCTTExt(XmlObject o) {
        if (o instanceof CTR) {
            return isCTTExt((CTR) o);
        }
        return false;
    }

    public boolean compareCTTExtStyle(CTR ctr1, XmlObject ctr2) {
        if (ctr2 instanceof CTR) {
            return compareCTTExtStyle(ctr1, (CTR) ctr2);
        }
        return false;
    }
    ...
    private CSSStyle getCssStyle(CTR ctr) {
        CSSStylesDocument cssStylesDocument = (CSSStylesDocument) getStylesDocument();

        CTRPr rPr = ctr.getRPr();
        CSSStyle cssStyle = cssStylesDocument.createCSSStyle(rPr);
        if (cssStyle != null) {
    ...
    protected String getCTTExt(CTR ctr) {
        ...
        while (c.toNextSelection()) {
            ...
            if (o instanceof CTText) {
                ...
                String value = ctText.getStringValue();
    ...
    private void visitRuns(XWPFParagraph paragraph, T paragraphContainer)
        ...
        while (c.toNextSelection()) {
            ...
            if (old != null && (!isCTTExt(o) || !compareCTTExtStyle(old, o))) {
                ...
                if (textsb.length() > 0) {
                    old.getTArray(0).setStringValue(textsb.toString());
                }
                Map<String, Object> data = runCTR(old, paragraph, paragraphContainer, fldCharTypeParsing, pageNumber, url, rListAfterSeparate);
            ...
            if (o instanceof CTOMathImpl) {
                System.out.println();
                visitRunCTOMathImpl(paragraph, (CTOMathImpl) o, paragraphContainer);
                isBeforeRun = false;
...
//                System.out.println("**************************************************************************************");
    ...
    private Map<String, Object> runCTR(CTR r, XWPFParagraph paragraph, T paragraphContainer,
                                       boolean fldCharTypeParsing,
                                       boolean pageNumber,
                                       String url,
                                       List<XmlObject> rListAfterSeparate) throws Exception {
        ...
        result.put("fldCharTypeParsing", fldCharTypeParsing);
        result.put("pageNumber", pageNumber);
        result.put("url", url);
        result.put("rListAfterSeparate", rListAfterSeparate);
        return result;
    }
